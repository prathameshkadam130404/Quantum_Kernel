...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(os.path.join(SAVE_DIR, "exp_e33.log"), mode="w"),
    ],
)
...
def _build_kernel(
    ...
    # -- Fallback: reuse kernels from prior experiments ----------------------
    _fallback_map = {
    ...
    fallback = _fallback_map.get((method, dataset, fset))
    if fallback is not None and os.path.exists(fallback):
        K = np.load(fallback)
        if K.shape[0] == len(X_enc):
            log.info("  [REUSE] Loaded %s from prior experiment: %s", desc, fallback)
            os.makedirs(os.path.dirname(cache), exist_ok=True)
            np.save(cache, K)
            return K
        else:
            log.warning(
                "  [REUSE] Shape mismatch for %s: expected %d, got %d. Recomputing.",
                desc, len(X_enc), K.shape[0],
            )
    ...
    if method == "SG-BSCM":
        ...
        return compute_srqfm_fidelity_kernel(
            ...
            coupling_threshold=0.01 if dataset == "so2sat" else 0.0,
            connectivity=conn,
            kernel_save_path=cache, desc=desc,
        )
    else:
